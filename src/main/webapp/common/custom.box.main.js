				...
				callback : function(data) {
					if (data) {
						for (var m = 0; m < data.length; m++) {
							 ...
							 * selectobj.options[selectobj.length-1].selected =
							 * true; }
							 */
						}
					}
...
/**
 * 用于向业务代码的数据字典下拉框中加载数据 typecode为数据字典的typecode id：select下拉框的id
 * 
 */
...
createSyncTree.prototype.showMenu = function() {
	...
	this.treeareaObj.css({
				...
				top : this.textObjOffset.top + this.textObj.outerHeight()
						+ "px"
			}).slideDown("fast");
	// 绑定鼠标按下事件
