...
<script type="text/javascript">
	var columns_v = [ [{
		...
		align : 'center',
		formatter : function(value, row, index) {//通过此方法格式化显示内容,value表示从json中取出该单元格的值，row表示这一行的数据，是一个对象,index:行的序号
			return "<a href='javascript:;' style='font-size:14px;' onclick='dotask(\""+row.id+"\",1)'>【提交任务】</a>";
		}
	}] ];
	function showBug(id){
		createmodalwindow("查看任务", 600, 320, '${baseurl}task/showBug.action?id='+id);
	}
	function dotask(taskid,flag){
	    createmodalwindow("上传文件", 400, 122, '${baseurl}task/tofileupload.action?taskid='+taskid); 
}
